...
void MPU::readBytes(uint8_t address, uint8_t subAddress, uint8_t count, uint8_t * dest)
{
    ...
    void MPU::initAK8963(float * destination)
    {
    ...
    void MPU::calibrateMPU9250(float * dest1, float * dest2)
    {
    ...
    void MPU::MPU9250SelfTest(float * destination) // Should return percent deviation from factory trim values, +/- 14 or less deviation is a pass
    {
    ...
    void MPU::MadgwickQuaternionUpdate(float ax, float ay, float az, float gx, float gy, float gz, float mx, float my, float mz)
    {
    ...
    // measured ones.
    void MPU::MahonyQuaternionUpdate(float ax, float ay, float az, float gx, float gy, float gz, float mx, float my, float mz)
    {
        float q1 = q[0], q2 = q[1], q3 = q[2], q4 = q[3];   // short name local variable for readability
        float norm;
        float hx, hy, bx, bz;
    ...
    void MPU::magcalMPU9250(float * dest1, float * dest2)
    {
        uint16_t ii = 0, sample_count = 0;
        int32_t mag_bias[3] = {0, 0, 0}, mag_scale[3] = {0, 0, 0};
        int16_t mag_max[3] = {0x8000, 0x8000, 0x8000}, mag_min[3] = {0x7FFF, 0x7FFF, 0x7FFF}, mag_temp[3] = {0, 0, 0};

        if (VERBOSE) Serial.println("Mag Calibration: Wave device in a figure eight until done!");
        ...
        if (VERBOSE) Serial.println("Mag Calibration done!");
    }
